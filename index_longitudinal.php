...
http://redcap.stanford.edu/plugins/summarize/index_longitudinal.php?token_user=rogupta&if=study_coordinator_intake&df=data_summarize_output&xf=data_summarize_output&ie=triage_arm_1&de=triage_arm_1
	...
	- ie = input event (e.g. enrollment_arm_1)
	- df = destination field name for name of field to write output to (assumes classical project as this time)
	- de = destination event (for longitudinal projects)
	- xf = excluded fields (comma-delimited) to exclude from report

	example:  https://redcap.stanford.edu/plugins/summarize/?token_user=xxxxxx&if=form_a,form_b&df=form_a_b_summary&xf=record_id,salary

	
**/	

define('LOG_PREFIX', '/var/log/redcap/summarize_longitudinal');
define('LOG_DEBUG_ENABLED', TRUE);
define('LOG_DEBUG_SCREEN', FALSE);

logIt('POST: ' . print_r($_POST,true), "DEBUG");
logIt('GET: ' . print_r($_GET,true), "DEBUG");
...
$input_event = voefr('ie');
$destination_field = voefr('df');
$destination_event = !empty($_POST['de']) ? voefr('de') : $input_event;	// default to input event if not otherwise specified
...
// Error out if longitudinal and not specified properly
if (REDCap::isLongitudinal() && (!$input_event)) {
	logIt("Skipping - calling in longitudinal project withoput specifying input_event (ie)", "DEBUG");
	exit();	
}
...
// Get Data
$data = REDCap::getData('array', $record, $fields, (REDCap::isLongitudinal() ? $input_event : NULL), NULL, FALSE, FALSE, FALSE, NULL, TRUE);
...
if (REDCap::isLongitudinal()) {
	$data['redcap_event_name'] = $destination_event;
}
logIt("WriteToApi Data: " . json_encode($data),"DEBUG");

$result = writeToApi($data);
logIt("WriteToApi Result: " . json_encode($result),"DEBUG");

logIt("Summarization to $destination_field " . (REDCap::isLongitudinal() ? " in event $destination_event" : '') . " for record $record","INFO");
